...
// end shop
let ourcategory = document.querySelector(".ourcategory");
let ourcategoryContent = ourcategory.querySelector(".content");
let ourcategoryContentArray = ourcategoryContent.querySelectorAll(".box");
let products = document.querySelector(".products");
let productsArray = document.querySelectorAll(".contain");

ourcategory.addEventListener("click", function (n) {
  if (
    n.target.classList.contains("box") &&
    !n.target.classList.contains("active")
  ) {
    let target = n.target.getAttribute("data-target");
    for (let i = 0; i < ourcategoryContentArray.length; i++) {
      ourcategoryContentArray[i].classList.remove("active");
    }
    n.target.classList.add("active");
    products.querySelector(".contain.active").classList.remove("active");
    products.querySelector(target).classList.add("active");
  }
});
